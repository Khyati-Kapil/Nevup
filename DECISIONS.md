...
## Seeded mock API (CSV-backed) instead of purely synthetic responses
Track 3 requires data to come from the provided seed file. I added a dedicated local mock API service that loads `data/nevup_seed_dataset.csv` at startup and computes session/profile/metrics responses from that source. This keeps frontend behavior realistic and reviewable without relying on hardcoded fixture objects in UI code.

## Contract-first API integration with normalization guards
I introduced an API contract layer (`zod` parsing + normalization) before data reaches UI components. The mock API can return partial fields; normalizing input at the boundary reduces runtime UI failures, keeps component logic simple, and helps enforce predictable empty/error states.
...
I added `docker-compose.yml` with both the seeded mock API and frontend app so reviewers can run a single `docker compose up` command. This aligns with the global rule of no manual multi-step setup and keeps local evaluation close to judging conditions.
